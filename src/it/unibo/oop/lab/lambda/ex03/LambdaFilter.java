...
public final class LambdaFilter extends JFrame {
    ...
    private enum Command {
        ...
        COUNTCHAR("Count number of characters", (s -> Integer.toString(s.length()))),
        COUNTLINES("Count number of lines", (s -> Long.toString(s.chars()
                                                                 .filter(c -> c == '\n')
                                                                 .count() + 1))),
        LISTALPHABETICAL("Words in alphabetical order", (s -> Arrays.stream(s.split(ANY_NON_WORD))
                                                                    .sorted()
                                                                    .collect(Collectors.joining("\n")))),
        COUNTWORDS("Count words", (s -> Arrays.stream(s.split(ANY_NON_WORD))
